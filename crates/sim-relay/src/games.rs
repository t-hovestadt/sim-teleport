...
pub static GAMES: &[GameDef] = &[
    ...
    GameDef {
        ...
        console: false,
        notes: "sim-teleport source creates config.json automatically on first detection \
                (if the game has been run at least once). Manual path: \
                %USERPROFILE%\\Documents\\My Games\\Wreckfest 2\\<ProfileID>\\savegame\\telemetry\\config.json. \
                Restart the game after config.json is written.",
    },
